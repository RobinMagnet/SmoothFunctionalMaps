...
from .utils import fm_solvers, sm_solvers, spatial_solvers, opt_utils, params_utils, log_utils

class SmoothDiscreteOptimization:
    """
    Defines the Smooth Discrete optimization class, a generalization of ZoomOut, which alternatively solves for functional maps and pointwise maps
    """
    ...
    def solve_Y_12(self):
        """
        Solves for the primal map from mesh1 to mesh2
        """
        self.Y_12 = sm_solvers.solve_Y21(mesh1=self.mesh2, mesh2=self.mesh1, p2p_21=self.p2p_12, p2p_12=self.p2p_21, params=self.sm_params, landmarks=None)
    
    def solve_Y_21(self):
        """
        Solves for the primal map from mesh2 to mesh1
        """
        self.Y_21 = sm_solvers.solve_Y21(mesh1=self.mesh1, mesh2=self.mesh2, p2p_21=self.p2p_21, p2p_12=self.p2p_12, params=self.sm_params, landmarks=None)
    ...
    def solve_p2p_12(self, precise=False, n_jobs=1):
        ...
        self.p2p_12 = spatial_solvers.solve_p2p_21_with_primal(mesh1=self.mesh2, mesh2=self.mesh1, FM_12=self.FM_21, Y_21=self.Y_12, FM_21=self.FM_12, Y_12=self.Y_21, descr1=self.descr2, descr2=self.descr1, params_sp=self.sp_params, params_sm=self.sm_params, n_jobs=n_jobs, precise=precise)
    ...
    def solve_p2p_21(self, precise=False, n_jobs=1):
        ...
        self.p2p_21 = spatial_solvers.solve_p2p_21_with_primal(mesh1=self.mesh1, mesh2=self.mesh2, FM_12=self.FM_12, Y_21=self.Y_21, FM_21=self.FM_21, Y_12=self.Y_12, descr1=self.descr1, descr2=self.descr2, params_sp=self.sp_params, params_sm=self.sm_params, n_jobs=n_jobs, precise=precise)

    @property
    def n_inner(self):
        return self.opt_params.get("n_inner", 1)
    
    def set_params(self, params=None, sp_params=None, sm_params=None, opt_params=None):
        """
        ...
        if params is not None:
            if not ((sp_params is None) and (opt_params is None) and (sm_params is None)):
                raise ValueError("You must not provide sp_params, sm_params, or opt_params if you provide the full parameters")

            params = params_utils.load_params(params)
            # print(params.keys())
            self.sp_params = params.sp_params
            self.sm_params = params.sm_params
            ...
            self.sm_params = params_utils.load_params(sm_params)
            self.opt_params = params_utils.load_params(opt_params)

        self.init_params = dict(sp_params=self.sp_params.copy(), sm_params=self.sm_params.copy(), opt_params=self.opt_params.copy())
    ...
    def _preprocess_meta_info_params(self):
        ...
        emb_types_sp = spatial_solvers._get_spectral_embedding_types_from_params(self.sp_params)
        
        meta_info_sp = {"meta_info": dict(FM_type=fm_type, emb_types=emb_types_sp)}
        self.sp_params = OmegaConf.merge(self.sp_params, OmegaConf.create(meta_info_sp))

        emb_types_sm = spatial_solvers._get_primal_embedding_types_from_params(self.sm_params)
        meta_info_sm = {"meta_info": dict(emb_types=emb_types_sm)}
        self.sm_params = OmegaConf.merge(self.sm_params, OmegaConf.create(meta_info_sm))
    ...
    def print_emb_sizes(self):
        ...
        emb_dim_K1, emb_dim_K2, emb_dim_p, emb_dim_bias = spatial_solvers._get_emb_dim_from_emb_types(self.sp_params.meta_info.emb_types)
    ...
    def solve_from_p2p(self, p2p_21=None, p2p_12=None, descr1=None, descr2=None, n_jobs=1, verbose=False):
        ...
        self.sm_params = self.init_params["sm_params"].copy()
        OmegaConf.set_struct(self.sm_params, False)
        ...
        smooth_weight_list = opt_utils.generate_weights_list(k_init=self.sm_params.get('sm_weight_range', [1,1])[0], nit=self.opt_params.nit, step=None, k_max=self.sm_params.get('sm_weight_range', [1,1])[1], log_space=True)
        ...
        for it, k in enumerate(pbar):
            ...
            self.sm_params.global_reweight = float(smooth_weight_list[it])
            # print(self.sm_params.global_reweight)
